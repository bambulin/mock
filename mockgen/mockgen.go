...
import (
	...
	"github.com/bambulin/mock/mockgen/model"
...
const (
	gomockImportPath = "github.com/bambulin/mock/gomock"
)
...
func main() {
	...
	if outputPackageName == "" {
		...
		outputPackageName = "mock" //sanitize(pkg.Name)
	}
...
func (g *generator) Generate(pkg *model.Package, outputPkgName string, outputPackagePath string) error {
	...
	g.p("// Code generated by Bambula's MockGen. DO NOT EDIT.")
	if g.filename != "" {
	...
	for _, intf := range pkg.Interfaces {
		if err := g.GenerateMockInterface(intfPkgName, intf, outputPackagePath); err != nil {
			return err
		}
	}
...
func (g *generator) GenerateMockInterface(intfPkgName string, intf *model.Interface, outputPackagePath string) error {
	mockType := g.mockName(intf.Name)
	longTp, shortTp := g.formattedTypeParams(intf, outputPackagePath)
	sort.Sort(byMethodName(intf.Methods))

	g.p("var _ %v%v = (*%v)(nil)", intfPkgName, intf.Name, mockType)
	g.p("")
	...
	g.GenerateMethodHandlers(mockType, intf, outputPackagePath, shortTp)
	g.out()
	g.p("}")
...
func (g *generator) GenerateMethodHandlers(mockType string, intf *model.Interface, pkgOverride, shortTp string) {
	for _, m := range intf.Methods {
		g.p("")
		g.GenerateMethodHandler(mockType, m, pkgOverride, shortTp)
	}
}

func (g *generator) GenerateMethodHandler(mockType string, m *model.Method, pkgOverride, shortTp string) {
	...
	g.p("%vHandler func(%v)%v", m.Name, argString, retString)
	g.p("%vHandlerCalled int", m.Name)
}
...
func (g *generator) GenerateMockMethod(mockType string, m *model.Method, pkgOverride, shortTp string) error {
	...
	g.p("%v.%v%v++", idRecv, m.Name, "HandlerCalled")
	g.p("return %v.%v%v(%v)", idRecv, m.Name, "Handler", callArgs)
